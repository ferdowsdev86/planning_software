...
| bryntum-react | `<BryntumScheduler>` wrapper, JSX config props, `useRef` instance access, StrictMode handling |
| bryntum-angular | `<bryntum-scheduler>` selector, `[prop]="..."` template binding, standalone component setup |
| bryntum-vue | `<bryntum-scheduler>` component, `v-bind` config spreading, `BryntumSchedulerProps` typing |
| bryntum-vanilla | Direct class import from `@bryntum/scheduler`, instantiation with `appendTo` |
