...
Include CSS for the Scheduler, FontAwesome (used for the built-in icons) and the theme you want to use on page:
...
6. Importing multiple Bryntum products in the same application
   ...
   Regular (full) bundles include the entire dependency chain. For example, `scheduler.module.js` includes Scheduler
   ...
   **Ok** — using thin bundles:
   ...
   See the [Multiple products guide](#Scheduler/guides/integration/javascript/multiple-products.md) for full details and
   dependency tables.
