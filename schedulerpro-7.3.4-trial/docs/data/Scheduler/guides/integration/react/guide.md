...
Please refer to the [Bryntum npm repository guide](#Scheduler/guides/npm-repository.md) for 
complete access information.

## Use Bryntum Scheduler with React
...
This guide provides a basic introduction to using Bryntum Scheduler with React.

### The React wrappers

The wrappers encapsulate Bryntum Scheduler and other Bryntum widgets as React components, exposing configuration
options, properties, features, and events. The wrapped Bryntum components are used the standard React way.

## View online demos

Visit our [online example browser](https://bryntum.com/products/scheduler/examples/?framework=react) to view demos of Bryntum Scheduler with React.

## Build and run local demos

Follow [this guide](#Scheduler/guides/download.md#distribution) to download the distribution zip.
...
the `scheduler*.d.ts` files located in the `build` folder in the distribution zip. 

The `scheduler*.d.ts` files contain a special config type that can be passed to the class constructor. 
...
## Using React wrappers with Bryntum Scheduler
...
Here is a list of the available wrappers for the Bryntum Scheduler React package.
...
The wrappers are provided in the `@bryntum/scheduler-react` package, which you can install using your preferred
package manager. Refer to the [guide to accessing the Bryntum npm repository](#Scheduler/guides/npm/repository/private-repository-access.md) 
for more details.

To use native API package classes with the wrappers, import them from `@bryntum/scheduler`:
...
The Bryntum Scheduler React wrapper defines a component named `BryntumScheduler`, which can be used like any 
...
Bryntum React wrappers for Bryntum Scheduler are bundled with TypeScript definitions that you can use 
...
For example, define the typed wrapper configuration for `BryntumScheduler` in `AppConfig.ts`:
...
The following example shows how to configure a `Toolbar` widget in the Bryntum Scheduler wrapper in `AppConfig.js`:
...
By default, the stores used by the Bryntum Scheduler React wrapper have 
...
Columns in Bryntum Scheduler support a [renderer](#Grid/column/Column#config-renderer) configuration option,
...
BryntumScheduler wrapper, so the wrapper must be used for them to work.
...
You can create custom React components to be rendered in Bryntum Scheduler cells.
...
Take a look at the **Basic Bryntum Scheduler demo** for detailed guidance on implementing this type of cell editor.
...
Please note that support for React components is implemented in the Bryntum Scheduler React wrapper. 
Applications that create the Bryntum Scheduler instance directly instead of using the wrapper cannot 
use JSX in tooltips and widgets.

Additionally, React components can only be used in tooltips and widgets that are children of Scheduler, 
...
See the <a href="https://bryntum.com/products/scheduler/examples/frameworks/react-vite/renderer-context/" target="_blank">React Vite recurring events demo</a>,
...
Features act as both configs and properties for `BryntumSchedulerComponent` and are mapped to the 
corresponding API features of the `instance`.

Feature names are suffixed with `Feature`.

Here is a comprehensive list of Bryntum Scheduler features:
...
Note that the React `BryntumScheduler` component is **not** the native Bryntum Scheduler
instance. It serves as a wrapper or interface between the React application and the Bryntum Scheduler.

While all available properties and features are propagated from the wrapper to the underlying 
Bryntum Scheduler instance, you can access the Bryntum Scheduler directly if necessary.

### Accessing the Bryntum Scheduler instance

You can access the Bryntum Scheduler instance like this:
...
To style the Bryntum Scheduler component, you can import a CSS file containing one of the Scheduler themes 
or enable theme switching with the `<BryntumThemeCombo />` component.

### Using a single theme

The easiest way to style the Bryntum Scheduler component is to import the CSS file in `App.js` or `App.scss`.
...
import '@bryntum/scheduler/visby-light.css';
import '@bryntum/scheduler/visby-dark.css';
...
import '@bryntum/scheduler/material3-dark.css';
```
...
<head>
    ...
    <!-- Structural CSS -->
    <link rel="stylesheet" href="%PUBLIC_URL%/themes/scheduler.css" />
    <!-- Bryntum theme -->
...
## Dynamically loading the Bryntum Scheduler component with Next.js
...
additional steps are required to dynamically load the Bryntum Scheduler component.

First, create a wrapper component for `BryntumScheduler` to ensure React refs function correctly.
...
## Best practices for using Bryntum Scheduler with React
...
We also recommend against copying the downloaded and unzipped Bryntum Scheduler files into your project tree, 
...
Refer to the [quick start guide on integrating Bryntum Scheduler with React](#Scheduler/guides/quick-start/react.md) 
to get started creating your application.

### Managing configuration for Bryntum Scheduler React wrappers

You can configure the wrapper component by passing options as React props.

There are two approaches to managing the configuration in Bryntum Scheduler React wrappers. You can combine 
...
Take a look at the [troubleshooting guide](#Scheduler/guides/integration/react/troubleshooting.md) 
if you run into any issues setting up Bryntum Scheduler with React. 

## Useful links

* Learn more about configuration options, features, events, and methods in the [Bryntum Scheduler API docs](#api)
