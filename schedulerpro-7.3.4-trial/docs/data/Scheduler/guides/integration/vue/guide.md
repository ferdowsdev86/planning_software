...
## Use Bryntum Scheduler with Vue
...
This guide provides a basic introduction to using Bryntum Scheduler with Vue.

## View online demos

Visit our [online example browser](https://bryntum.com/products/scheduler/examples/?framework=vue) to view demos of Bryntum Scheduler with Vue.
...
<template>
    <bryntum-scheduler
        ref="scheduler"
        tooltip="schedulerProps.tooltip"
...
export default {
    ...
    setup() {
        ...
        const config = reactive(schedulerProps);
...
As shown above you can assign values and bind to Vue data with `tooltip="schedulerProps.tooltip"` or `v-bind` option.
...
You will also need to import CSS files for Bryntum Scheduler. The ideal place for doing it is the beginning of
...
export default {
    ...
    setup() {
        ...
        return {
            schedulerProps : reactive(schedulerProps),
            extraData
        };
    }
};
...
```html
<template>
    <bryntum-scheduler ref="scheduler" v-bind="schedulerProps" />
</template>
