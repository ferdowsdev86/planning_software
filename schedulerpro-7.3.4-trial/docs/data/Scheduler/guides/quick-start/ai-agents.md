...
Here's a prompt for creating a basic Scheduler. We've tested the prompt using Claude Code with Claude Sonnet 4.6 (high effort) and Codex using GPT 5.5.

The prompt works best when the agent has access to the [Bryntum MCP Server](https://bryntum.com/products/scheduler/docs/guide/Scheduler/ai-features/mcp-server?utm_source=ai-quickstart) and the [Bryntum AI Agent skill](https://github.com/bryntum/skills). They fall back to fetching docs directly if neither is installed.
...
## Building a basic Bryntum Scheduler

This prompt creates a basic Bryntum Scheduler:
...
- Look up the latest docs: use the `search_bryntum_docs` MCP tool with `product: "scheduler"` if it's already installed, otherwise silently fall back to https://bryntum.com/products/scheduler/docs/?utm_source=ai-quickstart.
...
- **Licensed:** install the non-trial package directly: `npm install @bryntum/scheduler@<version>`.
- Framework wrapper - use the same version (no `-trial` suffix in either case), e.g. `@bryntum/scheduler-react`, `@bryntum/scheduler-angular`, `@bryntum/scheduler-vue-3`. Skip for vanilla JS.
...
- [React](#Scheduler/guides/quick-start/react.md)
- [Angular](#Scheduler/guides/quick-start/angular.md)
- [Vue](#Scheduler/guides/quick-start/vue-3.md)
- [Vanilla JavaScript](#Scheduler/guides/quick-start/javascript-npm.md)
- [Bryntum Scheduler API docs](#api)
