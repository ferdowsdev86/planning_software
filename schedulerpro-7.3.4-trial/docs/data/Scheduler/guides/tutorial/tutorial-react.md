...
<div class="note">
None of the CSS or React files (except for <code>App.jsx</code> and <code>App.css</code>)
generated by the scaffolding process are needed for this tutorial,
so you can safely delete them (and any imports of them).
</div>
...
To load data remotely, configure `crudManager` with a URL to load from. Modify the previous snippet so that it shows
the following:
