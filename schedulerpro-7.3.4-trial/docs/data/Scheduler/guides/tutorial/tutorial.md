...
Next, add the bryntum dependencies. If you have the Bryntum Scheduler distribution, it provides pre-built JavaScript 
bundles. Copy the `/build/scheduler.module.js`, `/build/scheduler.css` & `/build/svalbard-light.css` files to the 
**my-scheduler-app** root folder.
...
To load data remotely, configure `crudManager` with a URL to load from. Modify the previous snippet, adding the 
following:
