...
Trial packages depend on a corresponding `@bryntum/scheduler-lib-trial` package (e.g., `@bryntum/grid-lib-trial`) which
contains the trial license.

The `@bryntum/scheduler-lib-trial` packages are different in public (**npmjs.com**) and private Bryntum repositories.
If you switch between repositories, run a project cleanup including npm cache cleaning as described in the
[Project cleanup](#Scheduler/guides/npm/repository/troubleshooting.md#project-cleanup) guide.
...
```shell
yarn add @bryntum/scheduler@7.3.4
yarn add @bryntum/scheduler-angular@7.3.4
```
...
```shell
npm show @bryntum/scheduler versions
```
...
```shell
npm install @bryntum/scheduler@6.1.8-nightly.20250330
```
...
The benefit of using npm package aliasing for trial packages is that we create an alias for the `scheduler-trial` package
using the name of the licensed `scheduler` package. This means there is no need to change your application code after
...
1. Log in to the Bryntum npm repository using your licensed Customer Zone account as described in the
   [Login](#Scheduler/guides/npm/repository/private-repository-access.md#login) guide
...
3. Perform a project cleanup as described in the [Project cleanup](#Scheduler/guides/npm/repository/troubleshooting.md#project-cleanup)
   guide
