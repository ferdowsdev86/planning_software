...
Visit our [online example browser](https://bryntum.com/products/grid/examples/?framework=vue) to view demos of Bryntum Grid with Vue.
...
export default {
    ...
    setup() {
        const grid = ref(null);
        const config = reactive(gridProps);
...
As shown above you can assign values and bind to Vue data with `tooltip="gridProps.tooltip"` or `v-bind` option.
...
You will also need to import CSS files for Bryntum Grid. The ideal place for doing it is the beginning of
