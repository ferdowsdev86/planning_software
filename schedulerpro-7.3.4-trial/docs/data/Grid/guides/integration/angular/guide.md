...
Please refer to this [guide for Bryntum npm repository access](#Grid/guides/npm/repository/private-repository-access.md).
...
This guide provides a basic introduction to using Bryntum Grid with Angular.

## View online demos

Visit our [online example browser](https://bryntum.com/products/grid/examples/?framework=angular) to view demos of Bryntum Grid with Angular.
...
```shell
npm install @bryntum/grid-angular@7.3.4
```
...
```shell
npm install @bryntum/grid-angular-view@7.3.4
```
...
```shell
npm install @bryntum/grid-angular@npm@bryntum/grid-angular-view@7.3.4
```
...
package manager. Please refer to this [guide for Bryntum npm repository access](#Grid/guides/npm/repository/private-repository-access.md).
...
You will also need to import the structural CSS and a theme for Bryntum Grid, and if you are not replacing
...
The problem here is that the html of Bryntum Grid is not generated by Angular but by the underlying JavaScript code.
Therefore, the modified CSS classes are not understood and the Grid is not styled at all.

The solution would be to import Bryntum Grid theme in `styles.scss` which makes it global when no CSS class name
mangling takes place so they are understood and properly applied to Bryntum Grid. The styles defined in component's
(s)css file are still scoped to the component only and can be used for html which is not Grid.
