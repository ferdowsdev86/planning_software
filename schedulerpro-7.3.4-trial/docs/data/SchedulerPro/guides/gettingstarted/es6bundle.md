...
Include CSS for the SchedulerPro, FontAwesome (used for the built-in icons) and the theme you want to use on page:
...
If you get a `The Bryntum Scheduler Pro bundle was loaded multiple times by the application` error, it means that
...
6. Importing multiple Bryntum products in the same application
   ...
   Regular (full) bundles include the entire dependency chain. For example, `schedulerpro.module.js` includes SchedulerPro
   ...
   **Ok** — using thin bundles:
   ...
   See the [Multiple products guide](#SchedulerPro/guides/integration/javascript/multiple-products.md) for full details and
   dependency tables.
