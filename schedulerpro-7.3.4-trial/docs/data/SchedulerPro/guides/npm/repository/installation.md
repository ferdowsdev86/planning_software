...
```shell
yarn add @bryntum/schedulerpro@npm:@bryntum/schedulerpro-trial@7.3.4
yarn add @bryntum/schedulerpro-angular@7.3.4
```
...
"dependencies": {
  ...
  "@bryntum/schedulerpro-angular": "7.3.4"
}
```
...
[Framework wrappers](#SchedulerPro/guides/npm/repository/components.md#frameworks-wrappers) table.

## Trial packages dependencies

Trial packages depend on a corresponding `@bryntum/schedulerpro-lib-trial` package (e.g., `@bryntum/grid-lib-trial`) which
contains the trial license.

The `@bryntum/schedulerpro-lib-trial` packages are different in public (**npmjs.com**) and private Bryntum repositories.
If you switch between repositories, run a project cleanup including npm cache cleaning as described in the
[Project cleanup](#SchedulerPro/guides/npm/repository/troubleshooting.md#project-cleanup) guide.
...
```shell
yarn add @bryntum/schedulerpro@7.3.4
yarn add @bryntum/schedulerpro-angular@7.3.4
```
...
```shell
npm show @bryntum/schedulerpro versions
```
...
```shell
npm install @bryntum/schedulerpro@6.1.8-nightly.20250330
```
...
The benefit of using npm package aliasing for trial packages is that we create an alias for the `schedulerpro-trial` package
using the name of the licensed `schedulerpro` package. This means there is no need to change your application code after
...
1. Log in to the Bryntum npm repository using your licensed Customer Zone account as described in the
   [Login](#SchedulerPro/guides/npm/repository/private-repository-access.md#login) guide
...
3. Perform a project cleanup as described in the [Project cleanup](#SchedulerPro/guides/npm/repository/troubleshooting.md#project-cleanup)
   guide
