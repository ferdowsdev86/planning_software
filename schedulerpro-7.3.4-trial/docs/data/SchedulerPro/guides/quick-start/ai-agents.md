...
Here's a prompt for creating a basic Scheduler Pro. We've tested the prompt using Claude Code with Claude Sonnet 4.6 (high effort) and Codex using GPT 5.5.

The prompt works best when the agent has access to the [Bryntum MCP Server](https://bryntum.com/products/schedulerpro/docs/guide/SchedulerPro/ai-features/mcp-server?utm_source=ai-quickstart) and the [Bryntum AI Agent skill](https://github.com/bryntum/skills). They fall back to fetching docs directly if neither is installed.
...
## Building a basic Bryntum Scheduler Pro

This prompt creates a basic Bryntum Scheduler Pro:
...
- Look up the latest docs: use the `search_bryntum_docs` MCP tool with `product: "schedulerpro"` if it's already installed, otherwise silently fall back to https://bryntum.com/products/schedulerpro/docs/?utm_source=ai-quickstart.
...
- **Licensed:** install the non-trial package directly: `npm install @bryntum/schedulerpro@<version>`.
- Framework wrapper - use the same version (no `-trial` suffix in either case), e.g. `@bryntum/schedulerpro-react`, `@bryntum/schedulerpro-angular`, `@bryntum/schedulerpro-vue-3`. Skip for vanilla JS.
...
- [React](#SchedulerPro/guides/quick-start/react.md)
- [Angular](#SchedulerPro/guides/quick-start/angular.md)
- [Vue](#SchedulerPro/guides/quick-start/vue-3.md)
- [Vanilla JavaScript](#SchedulerPro/guides/quick-start/javascript-npm.md)
- [Bryntum Scheduler Pro API docs](#api)
