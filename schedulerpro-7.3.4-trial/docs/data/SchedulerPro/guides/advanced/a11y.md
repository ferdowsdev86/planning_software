...
When any `Field` is configured `clearable : true`, <kbd>Alt</kbd>+<kbd>Backspace</kbd> may be used to invoke the
clear trigger and clear the field while the field input is focused.
...
When a context menu is available (Such as when focused on a Grid cell or header, or on an event or task in a
scheduling widget), pressing <kbd>Shift</kbd>+<kbd>F10</kbd> will invoke the context menu.
...
The `Button` widget is keyboard accessible. If it has a configured `menu`, then <kbd>Enter</kbd> or <kbd>↓</kbd> will
activate the menu. <kbd>Escape</kbd> will hide the menu.

### Button with split

If a button has `split : true`, then <kbd>Enter</kbd> will invoke the click handler, and <kbd>↓</kbd> will activate
its menu.
...
When a SchedulerPro is configured to represent data as a tree, the following keystrokes manipulate the nodes:
