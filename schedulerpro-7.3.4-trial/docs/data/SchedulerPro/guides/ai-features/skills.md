...
| bryntum-react | `<BryntumSchedulerPro>` wrapper, JSX config props, `useRef` instance access, StrictMode handling |
| bryntum-angular | `<bryntum-schedulerpro>` selector, `[prop]="..."` template binding, standalone component setup |
| bryntum-vue | `<bryntum-schedulerpro>` component, `v-bind` config spreading, `BryntumSchedulerProProps` typing |
| bryntum-vanilla | Direct class import from `@bryntum/schedulerpro`, instantiation with `appendTo` |
