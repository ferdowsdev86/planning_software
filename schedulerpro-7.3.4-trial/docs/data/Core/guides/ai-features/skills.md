...
| bryntum-react | `<BryntumCore>` wrapper, JSX config props, `useRef` instance access, StrictMode handling |
| bryntum-angular | `<bryntum-core>` selector, `[prop]="..."` template binding, standalone component setup |
| bryntum-vue | `<bryntum-core>` component, `v-bind` config spreading, `BryntumCoreProps` typing |
| bryntum-vanilla | Direct class import from `@bryntum/core`, instantiation with `appendTo` |
