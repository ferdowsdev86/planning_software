...
### Lazy configuration properties
If a configuration property's changer results in an expensive startup cost, and is not needed until used by the
class in some way, then it should be set up as `lazy`.

This means that the raw configuration object is cached and *not* passed through the changer or updater until the property
is first accessed:
...
class MyWidget extends Widget {
    ...
    onSomeButtonClick() {
        // The List will be created just in time
        this.myList.showBy(this.element);
    }

    // This will only be called on first access of this.myList
    changeMyList(list, was) {
        // Widget class's reconfigure will either reconfigure a previous value, or create a new instance
        return List.reconfigure(was, list, this);
    }
}
```

### Automatic cleanup of configuration properties
If a property needs to be cleaned up, then it can be set up to be nullified at destruction time and
the config system will pass `null` to the changer when the object is destroyed:
...
class MyWidget extends Widget {
    ...
    changeResizeObserver(resizeObserver, was) {
        // Ensure any previous value is cleaned up (eg: when null is passed at destruction time)
        was?.disconnect();

        if (resizeObserver) {
            resizeObserver = new ResizeObserver(this.onElResize.bind(this));
...
For detailed information on creating custom UI widgets (classes that extend `Widget`), including the `compose()` method,
DomConfig, lifecycle methods, and styling, see the [Creating Widgets](#Core/guides/advanced/widgets.md) guide.
