...
Trial packages depend on a corresponding `@bryntum/core-lib-trial` package (e.g., `@bryntum/grid-lib-trial`) which
contains the trial license.

The `@bryntum/core-lib-trial` packages are different in public (**npmjs.com**) and private Bryntum repositories.
If you switch between repositories, run a project cleanup including npm cache cleaning as described in the
[Project cleanup](#Core/guides/npm/repository/troubleshooting.md#project-cleanup) guide.
...
```shell
npm install @bryntum/core@7.3.4
npm install @bryntum/core-angular@7.3.4
```
...
```shell
npm show @bryntum/core versions
```
...
```shell
npm install @bryntum/core@6.1.8-nightly.20250330
```
...
The benefit of using npm package aliasing for trial packages is that we create an alias for the `core-trial` package
using the name of the licensed `core` package. This means there is no need to change your application code after
...
1. Log in to the Bryntum npm repository using your licensed Customer Zone account as described in the
   [Login](#Core/guides/npm/repository/private-repository-access.md#login) guide
...
3. Perform a project cleanup as described in the [Project cleanup](#Core/guides/npm/repository/troubleshooting.md#project-cleanup)
   guide
