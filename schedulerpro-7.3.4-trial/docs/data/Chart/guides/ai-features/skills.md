...
| bryntum-react | `<BryntumChart>` wrapper, JSX config props, `useRef` instance access, StrictMode handling |
| bryntum-angular | `<bryntum-chart>` selector, `[prop]="..."` template binding, standalone component setup |
| bryntum-vue | `<bryntum-chart>` component, `v-bind` config spreading, `BryntumChartProps` typing |
| bryntum-vanilla | Direct class import from `@bryntum/chart`, instantiation with `appendTo` |
