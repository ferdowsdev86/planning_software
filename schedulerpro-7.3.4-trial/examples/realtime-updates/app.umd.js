...
class LoginWidget extends ProjectWebSocketSubscriberMixin(Container) {
    ...
    async login() {
        ...
        await this.project.wsSend('login', {
            login    : userName,
            password : ''
        });
...
class UsersContainer extends ProjectWebSocketSubscriberMixin(Container) {
    ...
    updateUsers(users, previousUsers) {
        const newUsers = users.filter(user => previousUsers ? !previousUsers.includes(user) : true),
        ...
        if (command === 'users') {
            ...
            for (const user of users) {
                ...
                if (!DataGenerator.namesWithAvatars.includes(name)) {
                    imageUrl = '../_shared/images/transparent-users/none.png';
                }
                else {
                    imageUrl = `../_shared/images/transparent-users/${name.toLowerCase()}.png`;
                }
                this.add({
                    type : 'widget',
                    cls  : 'ws-online-user',
...
const demoServer = /bryntum.com/.test(window.location.hostname) ? 'wss://dev.bryntum.com:8090/' : undefined,
        ...
        listeners : {
            async paint() {
                if (autoLogin) {
                    ...
                    await project.wsSend('login', {
                        login    : DataGenerator.generateName(),
                        password : ''
                    });
