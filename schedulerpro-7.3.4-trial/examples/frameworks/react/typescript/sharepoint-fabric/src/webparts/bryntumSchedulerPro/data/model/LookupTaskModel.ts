 ...
 */
export default class LookupTaskModel extends EventModel {

    public isTask : true;

    // Value can be stored as a comma separated string or an array
    private getValueAsIntArray(value: string | number[] | string[]) : number[] {
        return value === '' ? [] : (Array.isArray(value) ? value : value.split(',')).map(id => parseInt(id)).filter(n => !Number.isNaN(n));
    }
