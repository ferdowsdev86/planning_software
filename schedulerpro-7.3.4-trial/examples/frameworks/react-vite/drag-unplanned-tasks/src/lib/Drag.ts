...
type DragConfig = DragHelperConfig & {
    ...
    getSchedule?: () => SchedulerPro | undefined
    constrain   : boolean
...
export class Drag extends DragHelper {
    ...
    private _scheduleRef! : SchedulerPro;
    private _getSchedule! : () => SchedulerPro | undefined;
    private _scrollManagerFallback?: ScrollManager;

    get schedule(): SchedulerPro {
        const current = this._getSchedule?.();
        return ((current && !(current as any).isDestroyed) ? current : this._scheduleRef) as SchedulerPro;
    }

    set schedule(value : SchedulerPro) {
        this._scheduleRef = value;
    }

    get scrollManager(): ScrollManager {
        return ((this.schedule as any)?.scrollManager ?? this._scrollManagerFallback) as ScrollManager;
    }

    set scrollManager(value : ScrollManager) {
        this._scrollManagerFallback = value;
    }

    constructor(config : DragConfig) {
        super(config);
        this.grid                   = config.grid;
        this._scheduleRef           = config.schedule;
        this._getSchedule           = config.getSchedule ?? (() => config.schedule);
        this.constrain              = config.constrain;
        this.outerElement           = config.outerElement;
        this._scrollManagerFallback = config.schedule.scrollManager as ScrollManager;
    }
    ...
    override createProxy = (grabbedElement : HTMLElement) : HTMLDivElement => {
        ...
        if (schedule.isHorizontal) {
            ...
            Object.assign(proxy.style, {
                ...
                width     : `${(schedule as any).resourceColumnWidth - 2 * (schedule.resourceMargin as number)}px`
            });
        }

        // Fake an event bar
        proxy.classList.add('b-bryntum', 'b-sch-event-wrap', 'b-style-traced', 'b-unassigned-class', `b-sch-${schedule.mode}`);
    ...
    override onDrag = ({ event, context } : { event : any, context : any }) : void => {
        const
            { schedule }                    = this,
            ...
            doctor                          = context.target && schedule.resolveResourceRecord(context.target, [event.offsetX, event.offsetY]),
        ...
        context.newStartDate = newStartDate;
    ...
    override onDrop = async({ context } : { context : any }) => {
        ...
        if (context.valid) {
            const
                { appointments, element, relatedElements = [], doctor, newStartDate } = context,
            ...
            for (let i = 0; i < appointments.length; i++) {
                ...
                await schedule.scheduleEvent({
                    ...
                    startDate      : i === 0 ? newStartDate : appointments[i - 1].endDate,
