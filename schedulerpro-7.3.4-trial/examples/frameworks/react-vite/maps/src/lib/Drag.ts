import { DomHelper, DateHelper, DragHelper, Grid, SchedulerPro, ScrollManager, StringHelper, TimeAxisViewModel, ResourceModel, DragHelperListenersTypes, SchedulerResourceModel, SchedulerEventModel, CalendarModel } from '@bryntum/schedulerpro';
...
export default class Drag extends DragHelper {
    ...
    override createProxy = (grabbedElement: HTMLElement) => {
        const
            ...
            crossSize                           = (isHorizontal ? schedule.rowHeight : (schedule as any).resourceColumnWidth) - 2 * (schedule.resourceMargin as number),
            // In horizontal mode the time axis runs along the width; in vertical mode along the height
            axisSize                            = isHorizontal ? schedule.timeAxisSubGrid.width : schedule.timeAxisSubGrid.height as number;
    ...
    override onDrag : DragHelperListenersTypes['drag'] = ({ event, context : ctx }) => {
        ...
        context.valid = Boolean(resourceRecord && newStartDate &&
            ...
            (schedule.allowOverlap || schedule.isDateRangeAvailable(newStartDate, endDate, null, resourceRecord) &&
                ...
                (!calendar || (calendar as any).isWorkingTime(newStartDate, endDate, true))));
    ...
    override onDrop : DragHelperListenersTypes['drop'] = async({ context : ctx }) => {
        ...
        if (context.valid) {
            const
                ...
                dropDate                          = schedule.getDateFromCoordinate(coordinate + (element.querySelector('.b-sch-event-buffer-before') as HTMLElement)[bufferSizeProp], 'round', false);
