import { EventModel, SchedulerEventModel, ResourceModel, StringHelper, Scheduler, PercentColumn, TaskEditConfig } from '@bryntum/schedulerpro';
...
export const schedulerProps : BryntumSchedulerProProps = {
    ...
    columns : [
        ...
        },
        {
            ...
            renderer(data) {
                ...
                // Detect overlap
                if (record.events.some((task : SchedulerEventModel) =>
                ...
                const
                    duration   = record.events.reduce((total : number, task : SchedulerEventModel) => total += task.duration, 0),
    ...
    eventRenderer(data) {
        const renderData = data.renderData as { eventColor : string; style : string };
        const eventRecord = data.eventRecord as ProEvent;
        const resourceRecord = data.resourceRecord as ResourceModel;
        ...
        return [
            ...
            eventRecord.note ? {
                tag       : 'i',
                className : 'fa fa-sticky-note b-last',
                dataset   : {
                    btip : StringHelper.encodeHtml(eventRecord.note)
                }
            } : null
        ];
    }
