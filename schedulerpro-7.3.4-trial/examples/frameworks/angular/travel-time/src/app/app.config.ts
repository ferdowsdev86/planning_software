import { DomConfig, DateHelper, type Duration, EventModel, ResourceModel, SchedulerPro, StringHelper, Toast, Tooltip } from '@bryntum/schedulerpro';
...
// Custom classes use in application
...
export const schedulerProProps: BryntumSchedulerProProps = {
    ...
    eventBufferFeature  : {
        ...
        renderer({ eventRecord, preambleConfig, postambleConfig }) {
            ...
            const
                ...
                { preambleIcon, preambleCls, preambleText, postambleIcon, postambleCls, postambleText } = appEvent;
    ...
    eventTooltipFeature : {
        ...
        onShow({ source }) {
            ...
            if (lat && lon) {
                const map = appTooltip.map = Leaflet.map('eventmap', {
                    zoomControl : false,
                    zoom        : 13,
                    center      : [lat, lon]
                });
        ...
        template : ({ eventRecord }) => {
            ...
            return `<header>
                     ${appResource.get('image') ? `<img class="resource-image" src="./assets/users/${appResource.get('image')}.png"/>` : ''}
                ...
                <div class="event-info">
                    <div class="event-details">
                        <strong><i class="fa fa-calendar"></i>${StringHelper.encodeHtml(name)}</strong>
                        ...
                        <strong><i class="fa fa-car-side"></i>Travel time</strong>
                        <span>${preamble || ''} <i class="fa fa-arrow-right"></i></span>
                        <span>${postamble || ''} <i class="fa fa-arrow-left"></i></span>
                    </div>
                    <div id="eventmap"></div>
                </div>
            `;
