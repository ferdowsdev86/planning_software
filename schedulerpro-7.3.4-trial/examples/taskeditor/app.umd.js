...
class ProEvent extends EventModel {
    static get fields() {
        ...
        }, 'jobCost', 'contact', 'phone', 'priority'];
    }
}
...
const scheduler = new SchedulerPro({
    ...
    columns : [{
        ...
        // A custom getter defined in .lib/Resource.js
        editor    : false,
        sortable  : false,
        groupable : false,
        align     : 'center',
        ...
        renderer(renderData) {
            ...
            if (resource.events.some(task => task.isScheduled && !scheduler.eventStore.isDateRangeAvailable(task.startDate, task.endDate, task, resource))) {
                return [{
                    tag       : 'i',
    ...
    // CSS class needed to allow using primary color scale in styling
        renderData.cls.add('b-colorize');
        ...
        return [{
            className : 'desc',
            children  : [StringHelper.encodeHtml(task.name), {
                tag       : 'div',
                className : 'order',
