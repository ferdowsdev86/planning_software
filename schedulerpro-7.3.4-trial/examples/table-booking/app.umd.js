...
const statuses = ['not_arrived', 'arrived', 'in_bar', 'all_seated', 'paid', 'all_guests_left'];
class Reservation extends EventModel {
    ...
    static fields = [{
        ...
        convert    : value => value ? new Duration(typeof value === 'number' ? value + 'min' : value) : null
    }];
}
...
class Table extends ResourceModel {
    ...
    static fields = [{
        ...
        name    : 'name',
        convert : (val, data) => data === null || data === undefined ? undefined : data.id
    }];
   ...
   */
    ...
    get nextGuestName() {
        var _this$ongoingOrNextRe;
        return (_this$ongoingOrNextRe = this.ongoingOrNextReservation) === null || _this$ongoingOrNextRe === undefined ? undefined : _this$ongoingOrNextRe.reservedBy;
    }
    get nextReservationNbrGuests() {
        var _this$ongoingOrNextRe2;
        return (_this$ongoingOrNextRe2 = this.ongoingOrNextReservation) === null || _this$ongoingOrNextRe2 === undefined ? undefined : _this$ongoingOrNextRe2.nbrGuests;
    }
    get nextReservationStart() {
        var _this$ongoingOrNextRe3;
        return (_this$ongoingOrNextRe3 = this.ongoingOrNextReservation) === null || _this$ongoingOrNextRe3 === undefined ? undefined : _this$ongoingOrNextRe3.startDate;
    }
    get nextGuestCC() {
        var _this$ongoingOrNextRe4;
        return (_this$ongoingOrNextRe4 = this.ongoingOrNextReservation) === null || _this$ongoingOrNextRe4 === undefined ? undefined : _this$ongoingOrNextRe4.creditCard;
    }
    get referenceDate() {
...
class TableScheduler extends SchedulerPro {
        ...
        columns : [{
            ...
            children     : [{
                ...
                renderer        : ({
                    ...
                    var _record$getOngoingOrN;
                    return (_record$getOngoingOrN = record.getOngoingOrNextEvent({
                        startDate : scheduler.referenceDate,
                        endDate   : scheduler.timeAxis.endDate
                    })) === null || _record$getOngoingOrN === undefined ? undefined : _record$getOngoingOrN.reservedBy;
                }
            }, {
                text     : 'Next',
                type     : 'date',
                ...
                renderer : ({
                    ...
                    const refEvent = record.getOngoingOrNextEvent({
                        ...
                        nextFollowingEvent = record.getOngoingOrNextEvent({
                            startDate     : refEvent === null || refEvent === undefined ? undefined : refEvent.endDate,
                            endDate       : scheduler.timeAxis.endDate,
                            ignoreOngoing : true
                        });
    ...
    getConflictingEventIdMap() {
        const overlappingEventsByDate = this.getConflictingEventsMap(),
            conflictingEventIds = {};
    ...
    getConflictingEventsMap() {
        ...
        for (let d = 0; d < 7; d++) {
            const date = DateHelper.add(this.widgetMap.dateStrip.startDate, d, 'd'),
            ...
            dayEvents.forEach(eventA => {
                dayEvents.forEach(eventB => {
                    if (eventA !== eventB && eventA.resourceId === eventB.resourceId && DateHelper.intersectSpans(eventA.startDate, eventA.endDate, eventB.startDate, eventB.endDate)) {
    ...
    eventRenderer({
        ...
        const labels = [...eventRecord.tags, ...eventRecord.allergies];
        ...
        const domConfig = [{
            ...
            class    : 'b-name-ct',
            children : [eventRecord.isVip ? {
                tag     : 'i',
                class   : 'fa fa-star',
                dataset : {
                    btip : 'VIP Guest'
                }
            } : undefined, eventRecord.isBirthday ? {
                tag     : 'i',
                class   : 'fa fa-cake',
                dataset : {
                    btip : 'Birthday event'
                }
            } : undefined, eventRecord.tags.length > 0 ? {
                tag   : 'i',
                class : 'fa fa-tag'
            } : undefined, eventRecord.allergies.length > 0 ? {
                tag     : 'i',
                class   : 'fa fa-hand-dots',
                dataset : {
                    btip : 'Allergies: ' + eventRecord.allergies.join(', ')
                }
            } : undefined, {
                class : 'b-reserved-by',
    ...
    onEventDetailsToggle({
        value
    }) {
        this.rowHeight = value ? 65 : 30;
    }
    onEventConflictsToggle({
        value,
        source
    }) {
        if (value) {
    ...
    async onBeforeEventResizeFinalize({
        source: scheduler,
        context
    }) {
        const {
...
class DateStrip extends ButtonGroup {
    ...
    changeItems(items) {
        ...
        items.dateMenuButton.menu.project = (_this$up = this.up(hasProject)) === null || _this$up === undefined ? undefined : _this$up.project;
   ...
   */
    shiftRange(shiftIncrement) {
        var _this$pressed$;
        const pressedIndex = ((_this$pressed$ = this.pressed[0]) === null || _this$pressed$ === undefined ? undefined : _this$pressed$.dayIndex) || 0;
   ...
   */
    highlightConflicts(conflictMap) {
        this.items.forEach(btn => {
            if (btn.date) {
                ...
                btn.toggleCls('b-date-conflicts', (dateConflicts === null || dateConflicts === undefined ? undefined : dateConflicts.size) > 0);
                btn.tooltip = (dateConflicts === null || dateConflicts === undefined ? undefined : dateConflicts.size) > 0 ? `${DateHelper.format(btn.date, 'MMM DD')}: ${dateConflicts.size} conflict${dateConflicts.size ? 's' : ''}` : '';
            }
        });
    }
    ...
    updateSelectedDate(selectedDate) {
        ...
        if ((_this$widgetMap = this.widgetMap) !== null && _this$widgetMap !== undefined && _this$widgetMap.dateMenuButton) {
    ...
    reconfigureDay(date, button) {
        ...
        button.text = [{
            ...
            children : [DateHelper.format(date, 'ddd'), {
                tag   : 'i',
...
class SettingsPanel extends Panel {
    ...
    onTickWidthSliderChange({
        value
    }) {
        this.scheduler.tickSize = value;
    }
    onSeatingLengthChange({
        value
    }) {
