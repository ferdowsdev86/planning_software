...
class OrderForm extends Popup {
    ...
    doDestroy() {
        var _this$rootTemplates;
        (_this$rootTemplates = this.rootTemplates) === null || _this$rootTemplates === undefined || _this$rootTemplates.destroy();
        super.doDestroy();
    }
}
    ...
    static configurable = {
        ...
        features : {
            ...
            cellTooltip : {
                ...
                tooltipRenderer : ({
                    record: order
                }) => order.isScheduled ? StringHelper.xss`<h4>${order.name}</h4>This order is scheduled to finish by <b>${DateHelper.format(order.finishDate, 'MMM d, h A')}</b>` : `<h4>Type: ${order.templateName}</h4>Try dragging this order onto the schedule`
            }
        },
        tbar : [{
            text       : 'Add order',
