...
class UnplannedGrid extends Panel {
    ...
    static configurable = {
        cls         : 'b-unplanned-grid no-demo-app-style',
        // Opt out of shared demo app styling
        flex        : '0 1 400px',
        collapsible : true,
        minHeight   : 0,
        header      : false,
        ...
        columns : [{
            ...
            cell        : info => {
                var _info$getValue;
                return StringHelper.encodeHtml(((_info$getValue = info.getValue()) === null || _info$getValue === undefined ? undefined : _info$getValue.display_name) || '');
            }
        }, {
            id          : 'duration',
            header      : 'Duration',
        ...
        if (firstPaint) {
            ...
            thead.addEventListener('click', event => {
                ...
                if (th) {
                    ...
                    const column = (_this$_table = this._table) === null || _this$_table === undefined ? undefined : _this$_table.getColumn(th.dataset.columnId);
                    if (column) {
    ...
    renderTable() {
        ...
        me._table = table;
        const thead = me.contentElement.querySelector('.b-unplanned-table-head'),
    ...
    headers.map(({
      ...
      row.getVisibleCells().map(cell => {
          const renderFn = cell.column.columnDef.cell,
...
const unplannedGrid = new UnplannedGrid({
    ...
    listeners : {
        cellClick : ({
            ...
            // When an event bar is clicked, bring the marker into view and show a tooltip
            if (record.marker) {
                mapPanel === null || mapPanel === undefined || mapPanel.showTooltip(record, true);
            }
        }
    }
});
