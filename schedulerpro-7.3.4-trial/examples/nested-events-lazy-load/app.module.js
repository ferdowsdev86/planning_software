...
import { SchedulerPro, DateHelper } from '../../build/schedulerpro.module.js';

// Data fetching, replace with your own
const
    dataFetch     = fetch('./data/data.json').then(res => res.json()),
    getResources  = async({ startIndex, count }) => {
    ...
    getEventsData = async({ startIndex, count, startDate, endDate, getAssignments = false }) => {
        const
            resources      = (await getResources({ startIndex, count })).data,
    ...
    project : {
        resourceStore : {
            ...
            async requestData({ startIndex, count }) {
                return await getResources({ startIndex, count });
            }
        },
        eventStore : {
            tree     : true,
            ...
            async requestData({ startDate, endDate, startIndex, count }) {
                const data = await getEventsData({ startDate, endDate, startIndex, count });
                return {
                    data
                };
            }
        ...
        assignmentStore : {
            lazyLoad : true,
            async requestData({ startDate, endDate, startIndex, count }) {
                const data = await getEventsData({ startDate, endDate, startIndex, count, getAssignments : true });
                return {
                    data
                };
            }
