...
class TinyMceField extends RichTextField.mixin(Delayable) {
    ...
    updateValue(value, ...args) {
        ...
        if (!this.inputting) {
            var _this$editor;
            (_this$editor = this.editor) === null || _this$editor === undefined || _this$editor.load();
        }
    }
    ...
    destroyEditor() {
        var _me$editor, _me$toolbarContainer;
        const me = this;

        // Remove unconsumed listener if method was not called from doDestroy
        me.detachListeners('popup-hide');
        (_me$editor = me.editor) === null || _me$editor === undefined || _me$editor.destroy(); // Destroy the existing instance
        me.editor = null;

        // Remove the Firefox-only docked toolbar container (recreated on next paint if still needed)
        (_me$toolbarContainer = me.toolbarContainer) === null || _me$toolbarContainer === undefined || _me$toolbarContainer.remove();
    ...
    setupEditor(editor) {
        ...
        if (me.inline) {
            ...
            editor.once('focus', () => me.setTimeout(() => {
            ...
            me.detachListeners('popup-hide');
            popup === null || popup === undefined || popup.ion({
                name    : 'popup-hide',
                hide    : 'destroyEditor',
                once    : true,
                thisObj : me
    ...
    internalOnPaint() {
        ...
        if (me._applyFFInlineFix && !((_me$toolbarContainer2 = me.toolbarContainer) !== null && _me$toolbarContainer2 !== undefined && _me$toolbarContainer2.isConnected)) {
    ...
    owns(target) {
        return super.owns(target) || Boolean(target === null || target === undefined ? undefined : target.closest('.tox-tinymce'));
    }
}
...
scheduler.crudManager.load().then(() => scheduler.editEvent(scheduler.eventStore.first));