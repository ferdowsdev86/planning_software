...
class ExamplesApp {
  constructor() {
    ...
    const groupOrder = window.groupOrder || {
        Pro: 0,
        'Integration/Pro': 1,
        Basic: 2,
        Intermediate: 3,
      ...
      store = me.examplesStore = new Store({
        data: examples,
        fields: ['folder', 'rootFolder', 'fullFolder', 'group', 'title', 'overlay', 'version', 'build', 'since', 'thin', 'offline', 'id', 'updated', 'globalUrl', 'description', 'popular', {
          name: 'tab',
          defaultValue: 'js'
        }],
        groupers: [{
      ...
      items: {
        ...
        filterField: {
          ...
          filterFunction(record, value) {
            ...
            return value === null || value === undefined ? undefined : value.toLowerCase().split(' ').every(word => ['title', 'version', 'folder', 'description'].some(param => {
              var _record$param;
        ...
        thumbSize: {
          ...
          items: [{
            ...
            tooltip: 'Medium thumbs',
            icon: 'fa fa-image',
            cls: 'medium-thumb',
            value: 'medium-thumb',
            pressed: true
          }, {
            tooltip: 'Large thumbs',
            icon: 'fa fa-image',
            cls: 'large-thumb',
            value: 'large-thumb'
          }],
  ...
  filterDemos() {
    ...
    me.jumpTo.items = [{
      id: 'top',
      text: 'Top'
    }].concat(exampleStore.groupRecords.map(r => ({
      id: r.id,
        ...
        configs.push({
          ...
          children: [{
            ...
            children: [{
              ...
              className: 'versions',
              children: [(_example$version = example.version) === null || _example$version === undefined ? undefined : _example$version.split('+').map(v => ({
                className: {
                  version: 1
                },
                text: v.trim()
      ...
      click(event) {
        ...
        if (el.dataset.external !== 'true') {
          new Popup({
            ...
            html: me.buildTip + `The demo can be found in distribution folder: <div class="tip-folder"><i class="fa fa-folder-open"></i><b>` + (!me.isOnline ? `<a href="${el.dataset.linkUrl}">${el.dataset.linkText}</a>` : el.dataset.linkText) + '</b></div>',
            closeAction: 'destroy',
