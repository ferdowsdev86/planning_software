...
queueMicrotask(() => {
  ...
  if (!document.head.querySelector('link[data-bryntum-theme]') && !loc.pathname.includes('csp')) {
    const themeToUse = window.theme = qs.get('theme') || localStorage.getItem('b-example-new-theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'svalbard-dark' : 'svalbard-light'),
