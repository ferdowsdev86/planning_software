...
const
    ...
    scheduler             = new Scheduler({
            ...
            timeSelection : {
                ...
                headerRenderer({ timeRange }) {
                    return `<span class='b-selection-start'>${DateHelper.format(timeRange.startDate, 'LST')}</span>
                        <button class='fa fa-plus' data-btip='Create event' aria-label="Create event"></button>
        ...
        onTimeSelectionChange({ startDate, endDate }) {
            const
                { selectionLabel, selectionChangeAction } = this.widgetMap;
            ...
            const
                intersectingEvents = startDate && this.eventStore.getEvents({ startDate, endDate }),
        ...
        onTimeSelectionElementClick({ domEvent, startDate, endDate }) {
            if (domEvent.target.matches('button')) {
                const toAdd = this.resourceStore.getAvailableResources({ startDate, endDate }).map(resource => ({
                    name       : 'New task',
        ...
        onComboChange({ source }) {
            const { selectedTimeSpan } = this.features.timeSelection;
