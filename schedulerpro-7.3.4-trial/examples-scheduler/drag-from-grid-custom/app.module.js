...
import { SchedulerResourceModel, DateHelper, DragHelper, DomHelper, Toast, Tooltip, Combo, Scheduler, StringHelper, SchedulerEventModel, Grid, Splitter } from '../../build/schedulerpro.module.js';
...
class Drag extends DragHelper {
    ...
    onDrag({ event, context }) {
        const
            me                                = this,
            { schedule }                      = me,
            { task, target, enteredSchedule } = context,
            isOverSchedule                    = target?.closest('.b-time-axis-sub-grid');
        ...
        if (isOverSchedule) {
            const
                ...
                resource   = target && schedule.resolveResourceRecord(target, [event.offsetX, event.offsetY]);
            ...
            valid = (Boolean(startDate && resource) &&
...
class Schedule extends Scheduler {
    ...
    eventRenderer({ eventRecord, resourceRecord }) {
        ...
        return StringHelper.xss`
            <div class="b-event-header"><span>${eventRecord.name}</span><i class="fa fa-${eventRecord.icon}"></i></div>
...
class Task extends SchedulerEventModel {
    ...
    static fields = [
        { name : 'icon', defaultValue : 'fa fa-asterisk' }
    ];
