    ...
    static get fields() {
        return [
            // Use a nested store to contain the child events of a parent event
            {
                name       : 'agenda',
                type       : 'store',
                storeClass : SchedulerEventStore
            }];
    }
}
const
    ...
    normalizeAgendaItems = eventData => {
        ...
        (_eventData$agenda = eventData.agenda) === null || _eventData$agenda === undefined || _eventData$agenda.forEach(nestedEvent => {
    ...
    refreshAgendaDates = eventRecord => {
        ...
        (_eventRecord$agenda = eventRecord.agenda) === null || _eventRecord$agenda === undefined || _eventRecord$agenda.forEach(nestedEvent => {
    ...
    eventRenderer({
        ...
        if (agenda) {
            ...
            const nestedEventData = agenda.map(nestedTask => {
                const position = dateToPx(DateHelper.add(startDate, nestedTask.startOffset)),
