...
import { DateHelper, Scheduler, StringHelper } from '../../build/schedulerpro.module.js';
...
const scheduler = new Scheduler({
    ...
    columns : [
        { type : 'resourceInfo', text : 'Staff', field : 'name', width : 150 },
        {
            ...
            width      : 140,
            htmlEncode : false,
            renderer   : ({ record }) => `<div class="color-box b-bryntum b-color-${record.eventColor}"></div>${StringHelper.capitalize(record.eventColor)}`,
            editor     : {
                type        : 'combo',
                items       : Scheduler.eventColors,
                editable    : false,
                listItemTpl : ({ value }) => `<div class="color-box b-bryntum b-color-${value}"></div><div>${value}</div>`
            }
        }
    ],
