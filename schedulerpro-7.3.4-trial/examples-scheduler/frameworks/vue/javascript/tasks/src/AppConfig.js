 ...
 */
import { Popup, StringHelper, Tooltip } from '@bryntum/schedulerpro';
    ...
    columns : [
        {
            ...
            renderer({ record, value, size }) {
                ...
                if (record.isParent) {
                    const image = record.image !== false ? StringHelper.xss`<img class="profile-img" src="users/${record.name.toLowerCase()}.png" alt="${record.name}"/>` : '';
                ...
                // Other rows are clients
                    ...
                    // Client template
                    return StringHelper.xss`<div class="client-color b-color-${
                        record.eventColor
                    }"></div>${value}`;
                }
            }
