import { DateHelper, DomHelper, EventHelper, type SchedulerEventModel, GridFeatureManager, InstancePlugin, type SchedulerResourceModel, type ResourceTimeRangeModel, type ResourceTimeRangeStore, type Scheduler, Tooltip } from '@bryntum/schedulerpro';
import type { DailyRateModel } from './DailyRateStore';
...
class DaySelectorFeature extends InstancePlugin {
    ...
    onResourceTimeRangeMouseDown({ resourceRecord, resourceTimeRangeRecord, domEvent }: {
        ...
        if (resourceTimeRangeRecord) {
            ...
            (domEvent.target! as Element).closest('.b-sch-resource-time-range')?.classList.add('b-selected', 'b-first', 'b-last');
    ...
    async onDayMouseOver({ resourceTimeRangeRecord, domEvent }: { resourceTimeRangeRecord: ResourceTimeRangeModel; domEvent: MouseEvent }) {
        ...
        if (reservation) {
            Object.assign(reservation, {
                ...
                totalPrice : dayRangeInstances.reduce((pre, day) => pre + Number((day as DailyRateModel).pricePerNight), 0)
            });
        }

        const events = scheduler.eventStore.getEvents({ resourceRecord : resource, startDate, endDate }) as SchedulerEventModel[];
...
export default DaySelectorFeature;
