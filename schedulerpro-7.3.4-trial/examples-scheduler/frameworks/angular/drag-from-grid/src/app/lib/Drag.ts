import {
    DateHelper, DomHelper, DragHelper, DragHelperConfig, DragHelperListenersTypes, Grid, Scheduler, ScrollManager, Toast, Tooltip
...
export class Drag extends DragHelper {
    ...
    override createProxy(element: HTMLElement): HTMLDivElement {
        const
            proxy        = document.createElement('div'),
            { schedule } = this,
            task         = this.grid.getRecordFromElement(element) as Task,
            durationInPx = schedule.timeAxisViewModel.getDistanceForDuration(task.getDurationMS());
    ...
    override onDrag: DragHelperListenersTypes['drag'] = ({ context, event }) => {
        const
            { schedule, tip } = this,
            { task }          = context as any,
    ...
    override onDrop: DragHelperListenersTypes['drop'] = ({ context }) => {
        const
            { schedule, tip, grid }    = this,
            { target, valid, element } = context,
            { task, resource }         = context as any;
        ...
        if (valid && target) {
            ...
            if (date) {
                ...
                (grid.store as TaskStore).remove(task);
                task.startDate = date;
    ...
    override onAbort: DragHelperListenersTypes['abort'] = () => {
        this.tip?.hide();
    };
