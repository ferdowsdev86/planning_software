...
const generator = DataGenerator.generate(1000),
...
class ColleagueSimulator extends Delayable() {
    construct(config) {
        ...
        me.scheduler.element.addEventListener('animationend', ({
            target,
            animationName
        }) => {
    ...
    removeResource() {
        ...
        (_this$resourceStore$l = this.resourceStore.last) === null || _this$resourceStore$l === undefined || _this$resourceStore$l.remove();
    }
    reassignEvent(event) {
        const otherResources = this.resourceStore.getRange().filter(r => r !== event.resource),
    ...
    getRandomResourceName() {
        ...
        return StringHelper.encodeHtml(((_this$resourceStore$g = this.resourceStore.getAt(Math.round(Math.random() * this.resourceStore.count))) === null || _this$resourceStore$g === undefined ? undefined : _this$resourceStore$g.name) || 'Mystery man');
    }
    async onResourceStoreChange({
        source,
        action,
        record,
        records
