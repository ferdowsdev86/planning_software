...
// Configure bundle paths for lazy loading (product bundles)
// Using absolute URLs to work with dynamic imports from any location
const
    buildPath   = new URL('../../build/thin/', import.meta.url).href,
    bundlePaths = {
        grid         : `${buildPath}grid.module.thin.js`,
        scheduler    : `${buildPath}schedulerpro.module.thin.js`,
        schedulerpro : `${buildPath}schedulerpro.module.thin.js`,
        gantt        : `${buildPath}gantt.module.thin.js`,
        calendar     : `${buildPath}calendar.module.thin.js`,
        taskboard    : `${buildPath}taskboard.module.thin.js`,
        chart        : `${buildPath}chart.module.thin.js`
    };

let cards;

// Initialize lazy loader (core + grid are imported statically)
LazyCardLoader.init(bundlePaths, { rootMargin : '600px' });
...
const app = new Container({
    ...
    items    : [
        ...
        // Widget display area
        {
            ...
            ref  : 'widgetArea',
            flex : 1,
            cls  : 'widget-grid columns-3' // Default to 3 columns
        }
    ]
});

// Get the grid container
const gridContainer = app.widgetMap.widgetArea.element;

// Function to create widget cards (lazy - widget created when visible)
function createWidgetCard(widgetInfo) {
    const card = DomHelper.createElement({
        className : {
            'widget-card'                         : 1,
            ['column-span-' + widgetInfo.colSpan] : widgetInfo.colSpan,
            ['row-span-' + widgetInfo.rowSpan]    : widgetInfo.rowSpan
        },
        dataset : {
            category    : widgetInfo.category,
            name        : widgetInfo.name.toLowerCase(),
            type        : widgetInfo.type,
            description : widgetInfo.description.toLowerCase()
        },
        children : [
            {
                ...
                children  : [
                    {
                        className : 'widget-card-header-top',
                        children  : [
                            widgetInfo.docsUrl ? {
                                tag       : 'a',
                                className : 'widget-card-header-title',
                                href      : 'https://bryntum.com/products/' + widgetInfo.docsUrl,
                                target    : '_blank',
                            ...
                            {
                                tag       : 'span',
                                className : 'widget-card-category',
                    ...
                    {
                        tag       : 'span',
                        className : 'widget-card-description',
            ...
            {
                className : {
                    'widget-card-content' : 1,
                    centered              : widgetInfo.centered
                },
                reference : 'content'
            }
        ]
    ...
    // Register for lazy loading instead of immediate creation
    LazyCardLoader.register(card, widgetInfo);

    return card;
}

function filterWidgets(searchText = '') {
    cards = cards || gridContainer.querySelectorAll('.widget-card');
    searchText = searchText.toLowerCase();

    cards.forEach(card => {
        const
            { name, category, type, description } = card.dataset,
            matches                               = !searchText ||
        ...
        card.classList.toggle('b-hidden', !matches);
    });
}
...
components.forEach(widget => {
    DomHelper.append(gridContainer, createWidgetCard(widget));
});

// Preload remaining bundles when idle, so they're ready when user scrolls
LazyCardLoader.preloadRemaining();
